...
def init_embeddings(embeddings_path, embeddings_image_path, image_dir):
    embeddings_df = pd.DataFrame(torch.load(embeddings_path, map_location=torch.device('cpu')).cpu())

    embeddings_image_paths = pd.read_csv(embeddings_image_path, header=None)
    embeddings_image_paths = embeddings_image_paths[0].str.split("/").str[-1]
    embeddings_df["img_name"] = embeddings_image_paths

    image_paths = os.listdir(image_dir)    

    embeddings_df = embeddings_df[embeddings_df["img_name"].isin(image_paths)]
    embeddings_df.drop("img_name", axis=1, inplace=True)
    ...
    le = LabelEncoder()
    class_number_label_list = le.fit_transform(class_label_list)

    embeddings_df["class"] = class_label_list
    embeddings_df["path"] = path_list
    embeddings_df["class_num"] = class_number_label_list
...
#check for center of labels, through plots
def calculate_concept_centers(embeddings_df):
    unique_class_names = sorted(embeddings_df["class"].unique())
    ...
    for class_name_index, class_name in enumerate(unique_class_names):
        print(class_name, embeddings_df[embeddings_df["class"] == class_name].shape)
        class_average = (
...
#could check for outliers, for loop -> find x ammount that are close, look at others that are Y distance or farther and nuke them. 
